...
| Telegram deploy Approve (opt-in) | `Support/DeployTelegramApprove.php` · `SiteUpdater::applyStagedZip` · `POST /telegram/deploy-webhook` + `/admin/updates/pending/{id}/approve` · env `TELEGRAM_DEPLOY_*` · mcp-cms `pending_approval` / `markPendingTelegram` · `UtilityPages` Updates · tests `DeployTelegramApproveTest.php` · `docs/deployment.md` |
