...
| Аналитика событий / целей | ZIP `modules-src/analytics/` (+ fixture `tests/fixtures/modules/analytics`); FE via `packageModuleLoader`; host page `admin/pages/AnalyticsAdminPage` (`hostPageKey: analytics.admin`); dashboard pulse `AnalyticsDashWidget` ← `modules/analytics/DashboardAnalyticsWidget` (fallback если package card не смонтировался); beacon → host slot `site.body.end` + consentBridge; jobs `$ctx->scheduler()` local `retention`/`aggregate`|
| Битые ??? лейблы пакетов (support) | BE `PluginCatalogMeta::isBrokenLabel` + `LABELS_RU` + `PackageModuleAdapter` (label/adminNav/settingsSchema); FE `AdminApp.resolveDisplayLabel` + `i18n` `support` |
...
| Dual-runtime PHP Shared ↔ Node VPS | Baseline `contracts/baseline/` · **behavior** `contracts/behavior/` + `scripts/behavior/{extract,generate,run-all,module-status,seed-dual-db}.mjs` · `php-router.php` loads packages via `InstalledModuleLoader` + `Bootstrap::addPackageModulesRoot(php-storage/modules)` · CI gate auth-only (`BEHAVIOR_REQUIRE=auth`, scenarios unauthenticated/invalid-token) until package migrations + happy-get green · parity `tests/parity/{behavior-runner,generated}/` · прогресс AUTO `docs/dual-runtime-parity-progress.md` · validate `scripts/contracts/validate-contracts.js` |
