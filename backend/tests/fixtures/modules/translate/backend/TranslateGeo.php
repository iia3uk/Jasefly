...
/**
 * Visitor country в†’ UI language for the public translate overlay.
 * Headers: Cloudflare / common CDN / Accept-Language fallback.
 */
final class TranslateGeo
{
    /** ISO 3166-1 alpha-2 в†’ BCP47-ish lang code used by the widget. */
     ...
     */
    public static function suggest(string $source, array $targets, ?PlatformRequestInterface $request = null): array
    {
        ...
        if ($via === 'default' || ($mapped !== null && $suggested !== $mapped && $suggested === 'en')) {
            // Mapped lang not available в†’ neutral en (or source if en unavailable).
    ...
    public static function resolve(?string $mapped, string $source, array $targets): string
    {
        ...
        // en not configured вЂ” still suggest en for overlay (batch allows it as neutral).
        return 'en';
    }

    public static function detectCountry(?PlatformRequestInterface $request = null): ?string
    {
        $headers = [
    ...
    public static function fromAcceptLanguage(?PlatformRequestInterface $request = null): ?string
    {
        ...
        // "fr-CH,fr;q=0.9,en;q=0.8" в†’ first primary tag
    ...
    private static function server(string $key, ?PlatformRequestInterface $request): ?string
    {
        if ($request) {
