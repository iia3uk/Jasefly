...
final class CommentsService
{
    ...
    public function create(array $input, ?string $ip = null): array
    {
        ...
        $this->db->run(
            'INSERT INTO comments (public_id,type,target_type,target_id,parent_id,user_id,author_name,author_email,body,rating,status,verified_purchase,ip_hash)
             VALUES (?,?,?,?,?,?,?,?,?,?,\'pending\',?,?)',
            [
                $publicId, $type, $targetType, $targetId, $parentId, $input['user_id'] ?? null,
                $name, $email !== '' ? $email : null, $body, $rating, $verified ? 1 : 0, $ipHash,
            ]
        );
        $comment = $this->db->one('SELECT * FROM comments WHERE id=?', [$this->db->lastInsertId()]) ?? [];
        $this->events->publish('comment.created', ['comment' => $this->safe($comment)]);
        if ($type === 'review') {
            $this->events->publish('review.created', ['comment' => $this->safe($comment)]);
        }
    ...
    public function moderate(int $id, string $status): array
    {
        ...
        if ($status === 'approved') {
            $this->events->publish('comment.approved', ['comment' => $this->safe($comment)]);
        }
        return $this->safe($comment);
    }
