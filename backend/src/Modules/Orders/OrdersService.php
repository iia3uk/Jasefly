...
final class OrdersService
{
    ...
    public function getCart(?string $publicId = null, ?int $userId = null): array
    {
        ...
        if (!$cart) {
            ...
            $cart = $this->db->one('SELECT * FROM carts WHERE id=?', [(int) $this->db->id()]);
        }
    ...
    {
        $items = $this->db->all(
            'SELECT ci.*, p.price current_price, p.title current_title, p.sku current_sku
             FROM cart_items ci LEFT JOIN products p ON p.id=ci.product_id WHERE ci.cart_id=? ORDER BY ci.id',
            [$cartId]
        );
        foreach ($items as &$item) {
    ...
    public function addNote(int $orderId, string $body, ?int $authorId = null, bool $visible = false): array
    {
        ...
        return $this->db->one('SELECT * FROM order_notes WHERE id=?', [(int) $this->db->id()]) ?? [];
    }
