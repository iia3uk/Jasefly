...
final class AnalyticsService
{
    ...
    public function cleanup(int $days): array
    {
        ...
        $events = $this->db->run("DELETE FROM analytics_events WHERE created_at < DATE_SUB(NOW(), INTERVAL ? DAY)", [$days])->rowCount();
        $sessions = $this->db->run("DELETE FROM analytics_sessions WHERE last_seen_at < DATE_SUB(NOW(), INTERVAL ? DAY)", [$days])->rowCount();
