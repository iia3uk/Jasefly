...
final class CommerceCatalog
{
    ...
    public function peek(string $type, int $id): ?array
    {
        ...
        $row = $this->db->one(
            "SELECT * FROM products WHERE id=? AND is_visible=1 AND $notDeleted",
            [$id],
        );
        if (!$row) {
            return null;
    ...
    public function resolve(string $type, int $id, ?int $variantIndex = null): ?array
    {
        ...
        if ($type === 'service') {
            ...
            $row = $this->db->one(
                "SELECT * FROM services WHERE id=? AND is_visible=1 AND is_purchasable=1 AND $notDeleted",
                [$id],
            );
        ...
        $notDeleted = $soft->notDeletedClause('products');
        $row = $this->db->one(
            "SELECT * FROM products WHERE id=? AND is_visible=1 AND is_purchasable=1 AND $notDeleted",
            [$id],
        );
        if (!$row) {
            return null;
    ...
    private function services(): array
    {
        try {
            ...
            $notDeleted = $soft->notDeletedClause('services');
            return $this->db->all(
                "SELECT * FROM services WHERE is_visible=1 AND is_purchasable=1 AND price IS NOT NULL AND price > 0 AND $notDeleted ORDER BY sort_order, id"
            );
        } catch (\Throwable) {
            return [];
        }
    ...
    private function products(): array
    {
        ...
        try {
            $soft = new SoftDeleteService($this->db);
            $notDeleted = $soft->notDeletedClause('products');
            // Visible products with a price — purchasability/stock checked at checkout.
            return $this->db->all(
                "SELECT * FROM products WHERE is_visible=1 AND price > 0 AND $notDeleted
    ...
    {
        try {
            return $this->db->inspector()->tableExists('products');
        } catch (\Throwable) {
            return false;
        }
    }
