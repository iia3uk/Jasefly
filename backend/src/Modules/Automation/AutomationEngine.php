...
final class AutomationEngine
{
    ...
    private function action(string $type, array $step, array &$context, int $runId, int $index, array $automation): array
    {
        ...
        return match ($type) {
            ...
            'update_submission' => $this->updateSubmission($config, $context),
    ...
    private function sendEmail(array $c, array $ctx): array
    {
        if (!class_exists(\App\Modules\Mail\Mailer::class)) {
            throw new \RuntimeException('Mail module unavailable');
        }
        ...
        (new \App\Modules\Mail\Mailer($settings, dirname(__DIR__, 3) . '/storage/logs'))->sendHtml(
            ...
            null,
            $this->render((string) ($c['text'] ?? $c['body'] ?? ''), $ctx)
        );
    ...
    {
        $settings = $this->pluginSettings('mail');
        $token = (string) ($c['bot_token'] ?? $settings['telegram_bot_token'] ?? '');
        $chat = (string) ($c['chat_id'] ?? $settings['telegram_chat_id'] ?? '');
        if ($token === '' || $chat === '' || !class_exists(\App\Modules\Mail\TelegramNotifier::class)) {
            throw new \RuntimeException('Telegram is not configured');
        }
        (new \App\Modules\Mail\TelegramNotifier($token, $chat, dirname(__DIR__, 3) . '/storage/logs'))
            ->send($this->render((string) ($c['text'] ?? ''), $ctx));
    ...
    private function sendWebhook(array $c, array $ctx): array
    {
        ...
        curl_setopt_array($ch, [
            CURLOPT_POST => true, CURLOPT_POSTFIELDS => $this->json($body),
            CURLOPT_HTTPHEADER => ['Content-Type: application/json'], CURLOPT_RETURNTRANSFER => true,
            CURLOPT_TIMEOUT => 8, CURLOPT_PROTOCOLS => CURLPROTO_HTTP | CURLPROTO_HTTPS,
        ]);
        ...
        if ($response === false || $code < 200 || $code >= 300) {
            throw new \RuntimeException($error ?: 'Webhook HTTP ' . $code);
        }
        return ['status' => $code];
    }
    ...
    private function notify(array $c, array $ctx): array
    {
        if (!class_exists(\App\Modules\Notifications\NotificationService::class)) {
            throw new \RuntimeException('Notifications module unavailable');
        }
        $svc = new \App\Modules\Notifications\NotificationService($this->db);
        $svc->notifyAdmins(
    ...
    private function updateSubmission(array $c, array $ctx): array
    {
        ...
        return ['submission_id' => $id, 'status' => $status];
    }
    ...
    {
        $row = $this->db->one('SELECT settings FROM modules WHERE name=? LIMIT 1', [$name]);
