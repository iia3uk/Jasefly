...
namespace App\PackageModules\CookieConsent\Hooks;
...
final class PostInstallHook implements ModuleHookInterface
{
    public function run(PlatformInstallContextInterface $context): void
    {
        ...
        @file_put_contents(
            $marker,
            gmdate(DATE_ATOM) . ' cookie-consent after_install v' . $context->version() . PHP_EOL,
            FILE_APPEND
        );
        $context->log('Cookie Consent PostInstallHook executed');
    }
}
