...
const PAGE_TEMPLATE_SLUGS = [

  { slug: 'login', title: 'Login', group: 'system', route: '/login' },

  { slug: '404', title: '404', group: 'system', route: '/404' },

  { slug: 'about', title: 'About', group: 'content', route: '/about' },

  { slug: 'contact', title: 'Contact', group: 'content', route: '/contact' },
...
export async function register(ctx: ModuleContext) {
  ...
  for (const p of ctx.apiPrefixes) {
    ...
    ctx.app.get(`${p}/admin/migrations`, ...gate, async (c) => ok(c, await runMigrations(ctx.db)));
    ...
    ctx.app.post(`${p}/admin/migrations/blueprints`, ...gate, async (c) => ok(c, await migrationBlueprints()));



    ctx.app.get(`${p}/admin/dashboard`, ...gate, async (c) => {
      ...
      if (await ctx.db.tableExists('users')) {

        const row = await ctx.db.one('SELECT COUNT(*) AS c FROM users');

        counts.users = Number(row?.c ?? 0);

      }

      if (await ctx.db.tableExists('contact_messages')) {

        const row = await ctx.db.one('SELECT COUNT(*) AS c FROM contact_messages');

        counts.messages = Number(row?.c ?? 0);
    ...
    ctx.app.get(`${p}/admin/plugins`, ...gate, async (c) => {
      ...
      return ok(c, { items: await ctx.db.all('SELECT * FROM modules ORDER BY name') });
    ...
    ctx.app.post(`${p}/admin/plugins/:name/toggle`, ...gate, async (c) => {
      ...
      if (['system', 'users'].includes(plug) && body.enabled === false) {
        ...
        return fail(c, 'Core module cannot be disabled', 422);
    ...
    ctx.app.get(`${p}/admin/system/status`, ...gate, async (c) =>
      ...
      ok(c, {
        ...
        storage_path: ctx.cfg.storagePath,
    ...
    ctx.app.get(`${p}/admin/system/last-error`, ...gate, async (c) => {
      ...
      return ok(c, report, 200, {
        message: report ? 'Последняя ошибка API' : 'Пока нет записанных ошибок',
      });
    ...
    ctx.app.get(`${p}/admin/updates`, ...gate, async (c) =>
      ...
      ok(c, {
        ...
        message: 'In-panel ZIP updates are handled by PHP SiteUpdater on shared hosting',
    ...
    ctx.app.get(`${p}/admin/content-pack/info`, ...gate, async (c) =>
      ...
      ok(c, {
        ...
        auth: {
          ...
          mcp_token: 'MCP_API_TOKEN → Authorization: Bearer <token>',
    ...
    ctx.app.get(`${p}/admin/page-templates`, ...gate, async (c) => {
      ...
      if (await ctx.db.tableExists('pages')) {
        ...
        for (const row of await ctx.db.all('SELECT id, slug, title, status, layout_json FROM pages')) {
      ...
      const items = PAGE_TEMPLATE_SLUGS.map((t) => {
    ...
    ctx.app.post(`${p}/admin/page-templates/ensure`, ...gate, async (c) =>
      ...
      ok(c, { created: 0, updated: 0, skipped: PAGE_TEMPLATE_SLUGS.length }),
    ...
    ctx.app.get(`${p}/admin/mcp/last-error`, mcp, async (c) => {
      ...
      return ok(c, report, 200, { broken: report !== null, message: report ? 'Есть last-error' : 'last-error пуст' });
