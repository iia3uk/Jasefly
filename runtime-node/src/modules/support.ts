...
async function listMessages(db: ModuleContext['db'], ticketId: number, afterId = 0) {
...
async function getTicketByPublicId(db: ModuleContext['db'], publicIdParam: string, visitorKey: string) {
...
async function supportSettings(db: ModuleContext['db']): Promise<Record<string, unknown>> {
  return moduleSettings(db, 'support');
}

async function hasOnlineAgents(db: ModuleContext['db']): Promise<boolean> {
...
async function touchAgent(db: ModuleContext['db'], userId: number): Promise<void> {
...
async function touchVisitor(db: ModuleContext['db'], ticketId: number): Promise<void> {
...
async function publicConfig(db: ModuleContext['db']) {
...
function agentGate(auth: AuthService) {
  return async (c: Context, next: () => Promise<void>) => {
    ...
    if (
      !user ||
      (!(await canSupport(auth, user, 'support.agent')) && !(await canSupport(auth, user, 'support.manage')))
    ) {
      return fail(c, 'Forbidden: support.agent required', 403);
    }
    await next();
  };
}

function manageGate(auth: AuthService) {
  return async (c: Context, next: () => Promise<void>) => {
    const user = c.get('user');
    if (!user || !(await canSupport(auth, user, 'support.manage'))) {
      return fail(c, 'Forbidden: support.manage required', 403);
    }
    await next();
  };
}
...
export async function register(ctx: ModuleContext) {
  ...
  const agent = [admin, perm, agentGate(ctx.auth)] as const;
  const manage = [admin, perm, manageGate(ctx.auth)] as const;

  for (const p of ctx.apiPrefixes) {
    ctx.app.get(`${p}/support/config`, async (c) => ok(c, await publicConfig(ctx.db)));

    ctx.app.get(`${p}/support/faq`, async (c) => {
      if (!(await ctx.db.tableExists('support_faq'))) return fail(c, 'capability_unavailable', 409);
    ...
    ctx.app.post(`${p}/support/faq/:id/ask`, async (c) => {
      ...
      if (ticket) {
        await insertMessage(ctx.db, Number(ticket.id), 'visitor', question);
        await insertMessage(ctx.db, Number(ticket.id), 'bot', answer);
        await ctx.db.run("UPDATE support_tickets SET status='bot', updated_at=? WHERE id=?", [nowSql(), ticket.id]);
      } else {
        ...
        const ticketId = await ctx.db.lastInsertId();
        await insertMessage(ctx.db, ticketId, 'visitor', question);
        await insertMessage(ctx.db, ticketId, 'bot', answer);
        ticket = await ctx.db.one('SELECT * FROM support_tickets WHERE id=?', [ticketId]);
      }

      const messages = await listMessages(ctx.db, Number(ticket!.id), 0);
      return ok(c, { ticket, messages }, 201);
    });
    ...
    ctx.app.post(`${p}/support/tickets`, async (c) => {
      ...
      if (existing) {
        await insertMessage(ctx.db, Number(existing.id), 'visitor', message);
        await touchVisitor(ctx.db, Number(existing.id));
        const messages = await listMessages(ctx.db, Number(existing.id), 0);
        return ok(c, { ticket: existing, messages }, 201);
      }

      const pid = newPublicId();
      const agentsOnline = await hasOnlineAgents(ctx.db);
      ...
      const ticketId = await ctx.db.lastInsertId();
      await insertMessage(ctx.db, ticketId, 'visitor', message);
      if (!agentsOnline) {
        await insertMessage(
          ctx.db,
          ticketId,
      ...
      const ticket = await ctx.db.one('SELECT * FROM support_tickets WHERE id=?', [ticketId]);
      const messages = await listMessages(ctx.db, ticketId, 0);
      return ok(c, { ticket, messages }, 201);
    });

    ctx.app.get(`${p}/support/tickets/:publicId`, async (c) => {
      if (!(await ctx.db.tableExists('support_tickets'))) return fail(c, 'Not found', 404);
      const key = sanitizeKey(c.req.query('visitor_key') || '');
      const ticket = await getTicketByPublicId(ctx.db, c.req.param('publicId'), key);
      if (!ticket) return fail(c, 'Not found', 404);
      return ok(c, ticket);
    });

    ctx.app.get(`${p}/support/tickets/:publicId/messages`, async (c) => {
      if (!(await ctx.db.tableExists('support_tickets'))) return fail(c, 'Not found', 404);
      ...
      if (!ticket) return fail(c, 'Not found', 404);
      await touchVisitor(ctx.db, Number(ticket.id));
      const messages = await listMessages(ctx.db, Number(ticket.id), afterId);
      return ok(c, { ticket, messages, agents_online: await hasOnlineAgents(ctx.db) });
    });

    ctx.app.post(`${p}/support/tickets/:publicId/messages`, async (c) => {
      const body = await readJsonBody(c);
      if (body instanceof Response) return body;
      ...
      if (agentsOnline) {
        await ctx.db.run("UPDATE support_tickets SET status='waiting_agent', updated_at=? WHERE id=?", [
          nowSql(),
          ticketId,
        ]);
      } else {
        await ctx.db.run("UPDATE support_tickets SET status='bot', updated_at=? WHERE id=?", [nowSql(), ticketId]);
      ...
      await touchVisitor(ctx.db, ticketId);
      const updated = await ctx.db.one('SELECT * FROM support_tickets WHERE id=?', [ticketId]);
      return ok(c, { message, bot_message: botMessage, ticket: updated }, 201);
    });

    ctx.app.post(`${p}/support/tickets/:publicId/contact`, async (c) => {
      const body = await readJsonBody(c);
      if (body instanceof Response) return body;
      ...
      await insertMessage(ctx.db, Number(ticket.id), 'system', 'Контакт сохранён', null);
      const updated = await ctx.db.one('SELECT * FROM support_tickets WHERE id=?', [ticket.id]);
      return ok(c, updated);
    });

    ctx.app.post(`${p}/support/heartbeat`, async (c) => {
      const body = await readJsonBody(c);
      if (body instanceof Response) return body;
      ...
      if (!publicIdParam || !key) return fail(c, 'public_id and visitor_key required', 422);

      const ticket = await getTicketByPublicId(ctx.db, publicIdParam, key);
      if (!ticket) return fail(c, 'Not found', 404);

      const settings = await supportSettings(ctx.db);
      if (leaving && settings.require_contact_on_leave !== false && !(await hasContact(ticket))) {
        await ctx.db.run("UPDATE support_tickets SET status='awaiting_contact', updated_at=? WHERE id=?", [
          nowSql(),
          ticket.id,
        ]);
      } else {
    ...
    ctx.app.post(`${p}/admin/support/presence`, ...agent, async (c) => {
      const user = getUserFromContext(c);
      if (!user || user === 'mcp') return fail(c, 'Unauthorized', 401);
      await touchAgent(ctx.db, Number(user.id));
      return ok(c, { ok: true, agents_online: await hasOnlineAgents(ctx.db) });
    });

    ctx.app.post(`${p}/admin/support/test-telegram`, ...manage, async (c) => {
      const settings = await supportSettings(ctx.db);
    ...
    ctx.app.get(`${p}/admin/support/tickets`, ...agent, async (c) => {
      ...
      if (status && status !== 'all') {
        ...
        items = await ctx.db.all('SELECT * FROM support_tickets ORDER BY updated_at DESC, id DESC LIMIT 100');
      }
    ...
    ctx.app.post(`${p}/admin/support/tickets/:id/messages`, ...agent, async (c) => {
      ...
      const message = await insertMessage(ctx.db, ticketId, 'agent', text, userId);
      if (ticket.status !== 'closed') {
        await ctx.db.run(
    ...
    ctx.app.post(`${p}/admin/support/tickets/:id/assign`, ...agent, async (c) => {
      const body = await readJsonBody(c);
      if (body instanceof Response) return body;
      const ticketId = Number(c.req.param('id'));
      const user = getUserFromContext(c);
      const uid = Number(body.user_id ?? (user && user !== 'mcp' ? user.id : 0));
      if (!uid) return fail(c, 'Validation failed', 422);
      await ctx.db.run('UPDATE support_tickets SET assigned_user_id=?, updated_at=? WHERE id=?', [
        uid,
        nowSql(),
        ticketId,
      ]);
    ...
    ctx.app.post(`${p}/admin/support/tickets/:id/close`, ...agent, async (c) => {
      const ticketId = Number(c.req.param('id'));
      const ticket = await ctx.db.one('SELECT * FROM support_tickets WHERE id=?', [ticketId]);
      if (!ticket) return fail(c, 'Not found', 404);
      await ctx.db.run("UPDATE support_tickets SET status='closed', updated_at=? WHERE id=?", [nowSql(), ticketId]);
      await insertMessage(ctx.db, ticketId, 'system', 'Тикет закрыт', null);
      const updated = await ctx.db.one('SELECT * FROM support_tickets WHERE id=?', [ticketId]);
      return ok(c, updated);
    });

    ctx.app.get(`${p}/admin/support/faq`, ...manage, async (c) => {
      if (!(await ctx.db.tableExists('support_faq'))) return ok(c, []);
      return ok(c, await ctx.db.all('SELECT * FROM support_faq ORDER BY sort_order ASC, id ASC'));
    });

    ctx.app.post(`${p}/admin/support/faq`, ...manage, async (c) => {
      if (!(await ctx.db.tableExists('support_faq'))) return fail(c, 'capability_unavailable', 409);
      const body = await readJsonBody(c);
      if (body instanceof Response) return body;
      const question = String(body.question ?? '').trim();
      const answer = String(body.answer ?? '').trim();
      if (!question || !answer) return fail(c, 'Вопрос и ответ обязательны', 422);
      await ctx.db.run(
      ...
      const item = await ctx.db.one('SELECT * FROM support_faq WHERE id=?', [id]);
      return ok(c, item, 201);
    });

    ctx.app.put(`${p}/admin/support/faq/:id`, ...manage, async (c) => {
      if (!(await ctx.db.tableExists('support_faq'))) return fail(c, 'Not found', 404);
      const body = await readJsonBody(c);
      if (body instanceof Response) return body;
      const id = Number(c.req.param('id'));
      const existing = await ctx.db.one('SELECT id FROM support_faq WHERE id=?', [id]);
      ...
      if (!sets.length) return fail(c, 'Нет полей', 422);
      sets.push('updated_at=?');
      params.push(nowSql(), id);
      await ctx.db.run(`UPDATE support_faq SET ${sets.join(', ')} WHERE id=?`, params);
      return ok(c, { message: 'OK' });
    });

    ctx.app.delete(`${p}/admin/support/faq/:id`, ...manage, async (c) => {
      if (!(await ctx.db.tableExists('support_faq'))) return fail(c, 'Not found', 404);
      await ctx.db.run('DELETE FROM support_faq WHERE id=?', [c.req.param('id')]);
