...
import { nowSql, okListOrEmpty, readJsonBody } from './_helpers.js';
...
async function resolveFormFields(db: ModuleContext['db'], form: Row): Promise<FormFieldDef[]> {
...
async function getFormById(db: ModuleContext['db'], id: string | number): Promise<Row | null> {
...
async function syncFormFields(db: ModuleContext['db'], formId: number, fields: unknown): Promise<void> {
...
export async function register(ctx: ModuleContext) {
  ...
  for (const p of ctx.apiPrefixes) {
    ctx.app.get(`${p}/forms/:slug`, async (c) => {
      ...
      const cols = await ctx.db.columns('forms');
      const deleted = cols.includes('deleted_at') ? ' AND deleted_at IS NULL' : '';
      const form = await ctx.db.one(`SELECT * FROM forms WHERE slug=?${deleted} LIMIT 1`, [
        c.req.param('slug'),
      ]);
      if (!form || String(form.status ?? 'active') !== 'active') return fail(c, 'Not found', 404);
      const fields = await resolveFormFields(ctx.db, form);
      return ok(c, {
        id: form.id,
        name: form.name,
        slug: form.slug,
    ...
    ctx.app.post(`${p}/forms/:slug/submit`, async (c) => {
      ...
      const pub = publicId();
      const cols = await ctx.db.columns('form_submissions');
      try {
        if (cols.includes('public_id') && cols.includes('payload')) {
          ...
          await ctx.db.run('INSERT INTO form_submissions (form_id, payload, created_at) VALUES (?, ?, ?)', [
            form.id,
            JSON.stringify(values),
            nowSql(),
          ]);
        } else {
          await ctx.db.run('INSERT INTO form_submissions (form_id, status, created_at) VALUES (?, ?, ?)', [
            form.id,
            'new',
            nowSql(),
          ]);
      ...
      const submissionId = await ctx.db.lastInsertId();
      await ctx.events.publish('form.submitted', {
        form_id: form.id,
        form_slug: form.slug,
    ...
    ctx.app.post(`${p}/admin/forms`, admin, async (c) => {
      if (!(await ctx.db.tableExists('forms'))) return fail(c, 'capability_unavailable', 409);
      ...
      await syncFormFields(ctx.db, id, body.fields);
      await ctx.events.publish('form.created', { form_id: id });
      const form = await getFormById(ctx.db, id);
      return ok(c, form, 201);
    });

    ctx.app.get(`${p}/admin/forms/:id`, admin, async (c) => {
      const form = await getFormById(ctx.db, c.req.param('id'));
      if (!form) return fail(c, 'Not found', 404);
      return ok(c, form);
    });

    ctx.app.put(`${p}/admin/forms/:id`, admin, async (c) => {
      ...
      params.push(id);
      await ctx.db.run(`UPDATE forms SET ${sets.join(', ')} WHERE id=?`, params);
      if (body.fields !== undefined) await syncFormFields(ctx.db, Number(id), body.fields);
      await ctx.events.publish('form.updated', { form_id: Number(id) });
      return ok(c, await getFormById(ctx.db, id));
    });

    ctx.app.delete(`${p}/admin/forms/:id`, admin, async (c) => {
      if (!(await ctx.db.tableExists('forms'))) return fail(c, 'Not found', 404);
      const id = c.req.param('id');
      const cols = await ctx.db.columns('forms');
      if (cols.includes('deleted_at')) {
        await ctx.db.run("UPDATE forms SET deleted_at=?, status='archived' WHERE id=?", [nowSql(), id]);
      } else {
        await ctx.db.run('DELETE FROM forms WHERE id=?', [id]);
      }
      await ctx.events.publish('form.deleted', { form_id: Number(id) });
      return ok(c, { ok: true });
    });

    ctx.app.get(`${p}/admin/forms/:id/submissions`, admin, async (c) => {
      if (!(await ctx.db.tableExists('form_submissions'))) return fail(c, 'capability_unavailable', 409);
    ...
    ctx.app.get(`${p}/admin/forms/:id/export`, admin, async (c) => {
      const form = await getFormById(ctx.db, c.req.param('id'));
      if (!form) return fail(c, 'Not found', 404);
      if (!(await ctx.db.tableExists('form_submissions'))) return fail(c, 'capability_unavailable', 409);
      ...
      for (const sub of subs) {
        ...
        if (await ctx.db.tableExists('form_submission_values')) {
          const vals = await ctx.db.all(
    ...
    ctx.app.get(`${p}/admin/form-submissions`, admin, async (c) => {
      return okListOrEmpty(c, ctx.db, 'form_submissions', async () => {
        const cols = await ctx.db.columns('form_submissions');
        const deleted = cols.includes('deleted_at') ? ' WHERE deleted_at IS NULL' : '';
        return ctx.db.all(`SELECT * FROM form_submissions${deleted} ORDER BY id DESC LIMIT 200`);
      });
    });

    ctx.app.get(`${p}/admin/form-submissions/:id`, admin, async (c) => {
      if (!(await ctx.db.tableExists('form_submissions'))) return fail(c, 'Not found', 404);
      const cols = await ctx.db.columns('form_submissions');
      const deleted = cols.includes('deleted_at') ? ' AND deleted_at IS NULL' : '';
      const sub = await ctx.db.one(`SELECT * FROM form_submissions WHERE id=?${deleted} LIMIT 1`, [
        c.req.param('id'),
      ]);
      if (!sub) return fail(c, 'Not found', 404);

      if (await ctx.db.tableExists('form_submission_values')) {
        sub.values = await ctx.db.all(
    ...
    ctx.app.put(`${p}/admin/form-submissions/:id`, admin, async (c) => {
      if (!(await ctx.db.tableExists('form_submissions'))) return fail(c, 'Not found', 404);
      const id = c.req.param('id');
      const cols = await ctx.db.columns('form_submissions');
      const deleted = cols.includes('deleted_at') ? ' AND deleted_at IS NULL' : '';
      const sub = await ctx.db.one(`SELECT * FROM form_submissions WHERE id=?${deleted} LIMIT 1`, [id]);
      if (!sub) return fail(c, 'Not found', 404);
      const body = await readJsonBody(c);
      if (body instanceof Response) return body;
      ...
      if (cols.includes('internal_note')) {
        await ctx.db.run('UPDATE form_submissions SET status=?, internal_note=COALESCE(?, internal_note) WHERE id=?', [
          status,
          body.internal_note ?? null,
          id,
        ]);
      } else {
        await ctx.db.run('UPDATE form_submissions SET status=? WHERE id=?', [status, id]);
      }
      if (status !== sub.status) {
        await ctx.events.publish('form.submission.status_changed', {
    ...
    ctx.app.post(`${p}/admin/form-submissions/bulk-status`, admin, async (c) => {
      if (!(await ctx.db.tableExists('form_submissions'))) return fail(c, 'capability_unavailable', 409);
      const body = await readJsonBody(c);
      if (body instanceof Response) return body;
      const ids = body.ids;
      ...
      if (!Array.isArray(ids) || !['new', 'in_progress', 'resolved', 'spam', 'archived'].includes(status)) {
        return fail(c, 'Invalid payload', 422);
      }
      const cols = await ctx.db.columns('form_submissions');
      const deleted = cols.includes('deleted_at') ? ' AND deleted_at IS NULL' : '';
      for (const sid of ids) {
        await ctx.db.run(`UPDATE form_submissions SET status=? WHERE id=?${deleted}`, [status, Number(sid)]);
      }
      return ok(c, { ok: true, count: ids.length });
    });

    ctx.app.delete(`${p}/admin/form-submissions/:id`, admin, async (c) => {
      if (!(await ctx.db.tableExists('form_submissions'))) return fail(c, 'Not found', 404);
      const id = c.req.param('id');
      const sub = await ctx.db.one('SELECT public_id FROM form_submissions WHERE id=?', [id]);
      const cols = await ctx.db.columns('form_submissions');
      if (cols.includes('deleted_at')) {
        await ctx.db.run("UPDATE form_submissions SET deleted_at=?, status='archived' WHERE id=?", [nowSql(), id]);
      } else {
        await ctx.db.run('DELETE FROM form_submissions WHERE id=?', [id]);
      }
      await ctx.events.publish('form.submission.deleted', {
        submission_id: Number(id),
