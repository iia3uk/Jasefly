...
export class AuthService {
  ...
  async verify2fa(challengeToken: string, code: string) {
    try {
      ...
      if (!user) return { ok: false as const, error: 'Invalid credentials', status: 401 };
      ...
      if (!okTotp) return { ok: false as const, error: 'Invalid credentials', status: 401 };
  ...
  async refresh(refreshToken: string) {
    try {
      ...
      if (!row) return { ok: false as const, error: 'Unauthorized', status: 401 };
  ...
  async mePayload(user: Row | 'mcp') {
    ...
    return {
      ...
      capabilities: await this.capabilitiesFor(user),
      roles: [String(user.role ?? 'admin')],
      is_super: ['admin', 'super_admin'].includes(String(user.role)),
      caps_version: 1,
    };
  }

  private async capabilitiesFor(user: Row): Promise<string[]> {
    // Baseline: admin gets broad caps; editors content.* — refined by Access module later
    if (String(user.role) === 'admin' || String(user.role) === 'super_admin') {
      return ['*'];
    }
    return ['content.view', 'content.create', 'content.update', 'media.manage'];
  }
