...
function CommentsInner(){
  const qc=useQueryClient(); const [status,setStatus]=useState('pending'); const [type,setType]=useState('')
  const rows=useQuery({queryKey:['comments',status,type],queryFn:async()=>unpack<Comment[]>(await api.get(`/admin/comments?status=${status}&type=${type}`))})
