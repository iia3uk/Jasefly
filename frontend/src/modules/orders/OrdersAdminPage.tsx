...
function OrdersInner() {
  ...
  const rows = useQuery({ queryKey:['orders',filter], queryFn:async()=>unpack<Order[]>(await api.get(`/admin/orders${filter?`?status=${filter}`:''}`)) })
  const detail = useQuery({ queryKey:['order',selected], enabled:!!selected, queryFn:async()=>unpack<Order>(await api.get(`/admin/orders/${selected}`)) })
  ...
  return <div className="space-y-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="font-heading text-2xl">Заказы</h1><p className="text-sm text-zinc-400">Статусы, состав, заметки и возвраты.</p></div>
    <div className="flex gap-2"><select value={filter} onChange={(e)=>setFilter(e.target.value)} className="rounded-lg border border-white/10 bg-zinc-900 p-2"><option value="">Все статусы</option>{statuses.map((s)=><option key={s}>{s}</option>)}</select><a className="button px-3 py-2 text-sm" href="/api/v1/admin/orders/export">CSV</a></div></div>
