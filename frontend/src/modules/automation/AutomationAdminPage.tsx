...
const EVENTS: Array<{ value: string; label: string }> = [
  { value: 'form.submitted', label: 'Отправлена форма сайта' },
  { value: 'form.submission.status_changed', label: 'Сменился статус заявки формы' },
  { value: 'order.created', label: 'Создан заказ' },
  { value: 'order.paid', label: 'Заказ оплачен' },
  { value: 'payment.completed', label: 'Платёж завершён' },
  { value: 'comment.created', label: 'Новый комментарий' },
  { value: 'subscriber.created', label: 'Новый подписчик рассылки' },
  { value: 'user.registered', label: 'Регистрация пользователя' },
  { value: 'content.published', label: 'Опубликован контент' },
  { value: 'webhook.received', label: 'Входящий webhook' },
]
...
function AutomationEditor() {
  ...
  const eventKnown = EVENTS.some((e) => e.value === form.trigger_type)
          ...
          {list.data?.map((item) => (
            ...
            >
              ...
              <div className="mt-0.5 text-xs text-zinc-500">
                {item.trigger_type} · {STATUS_OPTS.find((s) => s.value === item.status)?.label.split('—')[0].trim() || item.status}
              </div>
            </button>
          ))}
        </GlassPanel>
