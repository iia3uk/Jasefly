...
function OrdersInner() {
  ...
  const rows = useQuery({ queryKey:['orders',filter], enabled: pluginOn, queryFn:async()=>unpack<Order[]>(await api.get(`/admin/orders${filter?`?status=${filter}`:''}`)) })
  const detail = useQuery({ queryKey:['order',selected], enabled: pluginOn && !!selected, queryFn:async()=>unpack<Order>(await api.get(`/admin/orders/${selected}`)) })
  ...
  const exportCsv = async () => { setExporting(true); try { await api.download('/admin/orders/export', 'orders.csv') } finally { setExporting(false) } }
  return <div className="space-y-4"><AdminPageHero title="Заказы" hint="Статусы, состав, заметки и возвраты." eyebrow="Коммерция" accent="emerald" actions={<div className="flex gap-2"><select value={filter} onChange={(e)=>setFilter(e.target.value)} className="rounded-lg border border-white/10 bg-zinc-900 p-2"><option value="">Все статусы</option>{statuses.map((s)=><option key={s}>{s}</option>)}</select><GhostButton onClick={()=>void exportCsv()} disabled={exporting}>{exporting?'CSV…':'CSV'}</GhostButton></div>} />
